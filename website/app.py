...
def show_student_page(student_id):
    ...
    # Галочка "Требовать смену фото профиля"
    needs_photo_update = student.get("needs_avatar_update", False)
    new_photo_flag = st.checkbox(
        "📸 Требовать смену фото профиля",
        value=needs_photo_update,
        key=f"photo_flag_{student_id}"
    )

    if new_photo_flag != needs_photo_update:
        supabase.table("students").update({"needs_avatar_update": new_photo_flag}).eq("id", student_id).execute()
        st.success("Требование обновления фото установлено!" if new_photo_flag else "Требование снято")
        st.rerun()
...
def show_homeworks_page():
    ...
    with col1:
        ...
        if not homeworks:
            ...
            st.dataframe(
                df[["Название", "Номера из учебника", "Дата сдачи"]],
                use_container_width=True,
                hide_index=True
            )
    ...
    with col_a:
        ...
        if students:
            ...
            df_classes = pd.DataFrame([
                {"Класс": k, "Количество": v} for k, v in sorted(class_counts.items())
            ])
    ...
    with col_b:
        ...
        if submissions:
            ...
            df_status = pd.DataFrame([
                {"Статус": STATUS_LABELS.get(k, (k, ""))[0], "Количество": v} 
